...
width_basis: Number of nodes for basis type

list_shapes: Type and size of shapes to add
nb_shapes: The number of other shapes that should be added to the basis shape
m: size of each nb_shape we are adding

Returns:
	role_id: labels (discriminate between basis or added shape)
"""

def generate_graph(basis_shape, attached_shape, nb_shapes=10, width_basis=25, m=4):

    basis_type = basis_shape
    list_shapes = [[attached_shape, m]] * nb_shapes

    G, role_id, _ = build_graph(width_basis, basis_type, list_shapes, start=0)
    G = perturb([G], 0.1)[0]
    ...
    """
    num_features = 5
    feature_generator=featgen.ConstFeatureGen(np.ones(num_features, dtype=float))
    """
